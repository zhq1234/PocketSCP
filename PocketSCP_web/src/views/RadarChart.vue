    ...
    methods: {
        async initData() {
            ...
            await axios({
                ...
                if (res.status === 200) {
                    console.log('radar_data', res.data.data);
                    // let drugscore = res.data.data["frame_pocket_data"].map(item => item.drugscore) // 成药性得分
                    // let hydropscore = res.data.data["frame_pocket_data"].map(item => item.hydropscore) // 疏水性得分
                    // let polarityscore = res.data.data["frame_pocket_data"].map(item => item.polarityscore) // 极性得分
                    // let totalsasa = res.data.data["frame_pocket_data"].map(item => item.totalsasa) // SASA溶剂可及表面积
                    // let volume = res.data.data["frame_pocket_data"].map(item => item.volume) // 体积
                    // let alphaspmaxdist = res.data.data["frame_pocket_data"].map(item => item.alphaspmaxdist) //阿尔法球质心最大距离

                    that.arr = res.data.data["frame_pocket_data"]
                    // let result = [[], [], [],[],[],[]];
                    ...
                    that.drugscoreMax = res.data.data["rangeData"].drugscore.max
                    that.hydropscoreMax = res.data.data["rangeData"].hydropscore.max
                    that.polarityscoreMax = res.data.data["rangeData"].polarityscore.max
                    that.totalsasaMax = res.data.data["rangeData"].totalsasa.max
                    that.volumeMax = res.data.data["rangeData"].volume.max
                    that.alphaspmaxdistMax = res.data.data["rangeData"].alphaspmaxdist.max

                    // that.arrs.push(volumes)
                    // let frame_pockets = res.data.data["frame_pocket_data"].map(item => item.frame_pocket)
