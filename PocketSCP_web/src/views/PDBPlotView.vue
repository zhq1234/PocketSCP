...
export default {
  ...
  methods: {
    initMolStart() {
      ...
      that.optionBall = {
        customData: {
          url: "http://127.0.0.1:5050/molStar/molStarInit?frame_pocket=" + that.framePocket, format: "pdb",
    ...
    async initRequire() {
      ...
      await axios({
        ...
        if (res.status === 200) {
          that.frameMap = res.data.data["frameMap"];       // 每一帧的各个属性值
          ...
          that.barData = res.data.data["barData"]
          // 表示每组的宽度
    ...
    timeLineSlider(pocketData) {
      ...
      that.viewerInstance.visual.update({
        customData: {
          url: "http://127.0.0.1:5050/molStar/slider?frame_pocket=" + pocketData, format: 'pdb',
    ...
    PDBClick(node) {
      ...
      that.viewerInstance.visual.update({
        customData: {
          url: "http://127.0.0.1:5050/molStar/framePocket?frame_pocket=" + node, format: 'pdb',
        }, bgColor: {r: 255, g: 255, b: 255}, visualStyle: "molecular-surface",
      }, true)
    }
  }
