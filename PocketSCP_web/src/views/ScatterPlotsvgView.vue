  ...
  methods:{
    ...
    async  handleChange(){
      ...
      await axios({
        ...
        d3.select('#chart').select('svg').select('#content').selectAll('circle').remove();
       
        console.log(that.frameNode)
        console.log('111')
        console.log(res.data.data['select_pocket']);

        d3.select('#chart').select('svg').select('#content')
        .selectAll('circle')
    ...
    console.log(that.chartDom)
    console.log(d3.selectAll('.removeItem')['_groups'][0][0])
      ...
      that.yScale = d3.scaleLinear().domain([-that.maxData["maxY"], that.maxData["maxY"]]).range([that.height,0]);
      // that.yScale = d3.scaleLinear().domain([-that.maxData["maxY"], that.maxData["maxY"]]).range([500,0]);
      ...
      let filteredData = this.scatterPlotFramePocketData["forceData"].filter(d => {
        ...
        return framePocketNumber < 1000 && framePocketNumber % 1 === 0;
      });
        // 绘制点
  that.chartDom.selectAll('circle')
    // .data(that.scatterPlotFramePocketData["forceData"])
    .data(filteredData)
    .enter()
    .append('circle')
    ...
    .attr('fill', d => {
      ...
      // d3.select('#chart').select('svg').select('g').attr('transform', transform);
      }
      ...
      //   .on('zoom',that.zoomFun);
       ...
       const svg1=d3.select('#chart').select('svg')
       const g = svg1.select('#content');
      ...
      that.chartDom.call(that.pointer);
      //切换圈选
      d3.select('body')
      .on('keydown',() => {
    ...
    changeColorShow(e){
      ...
      console.log('change'+that.opacityK)
      d3.select('#chart').select('svg').select('#content').selectAll('circle').remove();
      ...
      .data(filteredData)
       ...
       .attr('r', 0.5)
       .attr('fill', d => {
        //  (res.data.data['select_pocket'].includes(d.frame_pocket)) 
          //  console.log('true')
          //  return 'rgba(0, 252, 200,1)'; // 高亮颜色
          ...
          //  let tempOpacity = ++that.densityMap[Math.floor(that.xScale(d.x) / that.densityGirdSize)][Math.floor(that.yScale(d.y) / that.densityGirdSize)] * that.opacityK / 2000 + parseFloat(that.minOpacity);
           let densityValue = ++that.densityMap[Math.floor(that.xScale(d.x) / that.densityGirdSize)][Math.floor(that.yScale(d.y) / that.densityGirdSize)];
          let tempOpacity = Math.pow(densityValue * that.opacityK / 500, 1.5) + parseFloat(that.minOpacity);  
          // let tempOpacity = Math.log(densityValue * that.opacityK / 2000 + 1) * 2 + parseFloat(that.minOpacity);        
           tempOpacity = tempOpacity > that.maxOpacity ? that.maxOpacity : tempOpacity;
           temp.push(tempOpacity);
