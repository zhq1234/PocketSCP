...
export default {
  ...
  methods: {
      ...
      await request({
        ...
        .then((res) => {
          if (res.status === 200) {
            ...
            this.framesNum = this.attrValueData.frame_num;
    ...
    myCharts() {
      ...
      // 生成1-100字符串的数组
      let frames = [];
      for (let i = 1; i <= this.framesNum; i++) {
        frames.push(i);
      }
      console.log(frames);
      ...
      let pocketColor = this.pocketColor;
    ...
    singleAxisDrop(event) {
      this.pocketData = event.dataTransfer
        ...
        .trim()
        .split(",");
      this.pocketColor = event.dataTransfer.getData("color");
      this.initData();
    },
    changeVolume() {
