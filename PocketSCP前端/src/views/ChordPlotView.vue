  ...
  data() {
    ...
    async initRequire() {
      ...
      if (that.chordDropData.length === 1) {
        ...
        await axios({
          url: "api/chordPlot/index",
          method: "post",
          data: {
            "frame_pocket": that.chordDropData, 
            "padAngle": that.padAngle
          }
        }).then(res => {
          if (res.status === 200) {
            that.chordPocketData = res.data.data
            // 颜色渐变
    ...
    drawCircleAndRing() {
      ...
      that.chart.append("path")
        ...
        .attr("stroke-width", 2)
        .attr("fill", "#BEBEBE")
        .attr("transform", `translate(${[that.width / 2 - 23, -that.height / 2 + 15]})`)
        .attr('cursor','pointer')
        .on("click", function () {
          console.log('click!');
