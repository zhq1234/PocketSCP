...
from PocketVIS.pocket_service import forcePlotService
...
def forcePlot_select_residue_pocket():
    print('select_residue_pocekt',request.get_data(as_text=True))
    # 力导向图初始化加载，只需加载位置即可
    print("/forcePlot/select_residue_pocket", request.get_data(as_text=True))

    return forcePlotService.forcePlot_select_residue_pocket(eval(request.get_data(as_text=True)))
