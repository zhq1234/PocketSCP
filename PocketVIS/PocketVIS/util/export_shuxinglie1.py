...
# data用来装shuxing
data=[]
def Jaccard( list_frame1,list_frame2 ):
    set_a = set(list_frame1)
    set_b = set(list_frame2)

    intersection_set_count = len(set_a.intersection(set_b))
    union_set_count = len(set_a.union(set_b))

    # 防止除数为0
    if union_set_count == 0:
        return 0
    else:
        return intersection_set_count / union_set_count
# 遍历每个文件
end=2000
for i in range(1, end):
    # 用filename文件读取每个口袋的属性
    filename = f'F:\\proj\\数据集\\新建文件夹\\allec_pdbs_out\\frame_{i}_out\\frame_{i}_info.txt'
     # 用filename1文件读取HETATM和ATOM
    filename1 = f'F:\\proj\\数据集\\新建文件夹\\allec_pdbs_out\\frame_{i}_out\\frame_{i}_out.pdb'
    ...
    with open(filename1, 'r') as file1:
        ...
        # print(frame_info)
        for j in range(1,  pocket_id+1):
            filename2 = f'F:\\proj\\数据集\\新建文件夹\\allec_pdbs_out\\frame_{i}_out\\pockets\\pocket{j}_atm.pdb'
            shuxing = []
            spheres=[]
            residualID=set({})
...
pd.DataFrame(data,columns=data_col).to_csv(f'F:\\BaiduNetdiskDownload\\5ht1b_2500_POCKET_2.csv',index=False)
# print(list_pockets)
# print(pockets['frame_pocket40_2'])
...
# list_pocket = pd.read_csv('D:\\研究生一年级\\分子动力学课题\\处理数据\\list_pocket_DhA_20001.csv')
# pockets[f'frame_pocket{list_pocket.iloc[i]['pdb_frame']}']['spheres']
# for i in range(1,total):
#     for j in range(i+1,total):
#         first=list_pocket.iloc[i]['pdb_frame']
#         second=list_pocket.iloc[j]['pdb_frame']
#         relate_value = Jaccard(pockets[f'frame_pocket{first}']['spheres'],pockets[f'frame_pocket{second}']['spheres'])
#         if relate_value >= 0.2:
#             ls_relation.append([first,second,round(relate_value,3)])
# pd.DataFrame(ls_relation,columns=ls_relation_col).to_csv(f'D:\\研究生一年级\\分子动力学课题\\处理数据\\source_target_weight_DhA300new.csv',index=False)
